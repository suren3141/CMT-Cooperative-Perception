...
from .cmt_head import pos2embed, LayerNormFunction, GroupLayerNorm1d, SeparateTaskHead

def filter_img_metas(img_meta, prefix='', ignore=''):
    """Update image_meta : Removes all keys starting with 'ignore' and removes prefixes of keys starting with 'prefix'
...
def get_infrastructure_image_metas(img_metas):

    infrastructure_img_metas = [filter_img_metas(img_meta, prefix='infrastructure_', ignore='vehicle_') for img_meta in img_metas]
    return infrastructure_img_metas

def get_vehicle_image_metas(img_metas):
    # TODO : Coordinate transformation - LiDAR to Image (image in vehicle cam coords, and LiDAR in infrastructure LiDAR coords)

    vehicle_img_metas = [filter_img_metas(img_meta, prefix='vehicle_', ignore='infrastructure_') for img_meta in img_metas]

    return vehicle_img_metas


@HEADS.register_module()
class CmtHeadCoop(BaseModule):
    ...
    def __init__(self,
        ...
        super(CmtHeadCoop, self).__init__(init_cfg=init_cfg)
    ...
    def init_weights(self):
        super(CmtHeadCoop, self).init_weights()
    ...
    def get_outs_dec(self, x, x_img, img_metas, reference_points, attn_mask):
    ...
    def forward_single(self, x_vehicle, x_infrastructure, x_img_vehicle, x_img_infrastructure, img_metas):
        """
            x: [bs c h w]
        ...
        reference_points, attn_mask, mask_dict = self.prepare_for_dn(len(img_metas), reference_points, img_metas)  # seperate for V and I ? Np?
        
        if x_vehicle is not None or x_img_vehicle is not None:    
            vehicle_image_metas = get_vehicle_image_metas(img_metas)
            out_dec_vehicle = self.get_outs_dec(x_vehicle, x_img_vehicle, vehicle_image_metas, reference_points, attn_mask)
        else:
            out_dec_vehicle = None
        if x_infrastructure is not None or x_img_infrastructure is not None:
            infrastructure_image_metas = get_infrastructure_image_metas(img_metas)
            out_dec_infrastructure = self.get_outs_dec(x_infrastructure, x_img_infrastructure, infrastructure_image_metas, reference_points, attn_mask)
        else:
            out_dec_infrastructure = None

        if out_dec_vehicle is None:
            outs_dec = out_dec_infrastructure
        elif out_dec_infrastructure is None:
            outs_dec = out_dec_vehicle
        else:
            outs_dec =  torch.stack([out_dec_vehicle, out_dec_infrastructure])
            outs_dec = torch.max(outs_dec, 0).values
    ...
    def forward(self, vehicle_pts_feats, infrastructure_pts_feats, vehicle_img_feats=None, infrastructure_img_feats=None, img_metas=None):
        """
            list([bs, c, h, w])
        """
        img_metas = [img_metas for _ in range(len(vehicle_pts_feats))]
        return multi_apply(self.forward_single, vehicle_pts_feats, infrastructure_pts_feats, vehicle_img_feats, infrastructure_img_feats, img_metas)
...
@HEADS.register_module()
class CmtImageHeadCoop(CmtHeadCoop):

    def __init__(self, *args, **kwargs):
        super(CmtImageHeadCoop, self). __init__(*args, **kwargs)
        self.shared_conv = None

    
    def get_outs_dec(self, x_img, img_metas, reference_points, attn_mask):
    ...
    def forward_single(self, x_vehicle, x_infrastructure, x_img_vehicle, x_img_infrastructure, img_metas):
        """
            ...
            return List(dict(head_name: [num_dec x bs x num_query * head_dim]) ) x task_num
        """
        assert x_vehicle is None and x_infrastructure is None
        ret_dicts = []
        
        reference_points = self.reference_points.weight
        reference_points, attn_mask, mask_dict = self.prepare_for_dn(len(img_metas), reference_points, img_metas)

        if x_img_vehicle is not None:
            vehicle_image_metas = get_vehicle_image_metas(img_metas)
            out_dec_vehicle = self.get_outs_dec(x_img_vehicle, vehicle_image_metas, reference_points, attn_mask)

        if x_img_infrastructure is not None:
            infrastructure_image_metas = get_infrastructure_image_metas(img_metas)            
            out_dec_infrastructure = self.get_outs_dec(x_img_infrastructure, infrastructure_image_metas, reference_points, attn_mask)

        if x_img_vehicle is None:
            outs_dec = out_dec_infrastructure
        elif x_img_infrastructure is None:
            outs_dec = out_dec_vehicle
        else:
            outs_dec =  torch.stack([out_dec_vehicle, out_dec_infrastructure])
            outs_dec = torch.max(outs_dec, 0).values
...
@HEADS.register_module()
class CmtLidarHeadCoop(CmtHeadCoop):

    def __init__(self, *args, **kwargs):
        super(CmtLidarHeadCoop, self). __init__(*args, **kwargs)
    ...
    def get_outs_dec(self, x, img_metas, reference_points, attn_mask):
        ...
        bev_pos_embeds = self.bev_embedding(pos2embed(self.coords_bev.to(x.device), num_pos_feats=self.hidden_dim))
        bev_query_embeds, _ = self.query_embed(reference_points, img_metas=None)    # bev_query_embeds doesn't require image_metas
    ...
    def forward_single(self, x_vehicle, x_infrastructure, x_img_vehicle, x_img_infrastructure, img_metas):
        """
            ...
            return List(dict(head_name: [num_dec x bs x num_query * head_dim]) ) x task_num
        """
        assert x_img_vehicle is None and x_img_infrastructure is None
        ...
        if x_vehicle is not None:
            out_dec_vehicle = self.get_outs_dec(x_vehicle, img_metas, reference_points, attn_mask)

        if x_infrastructure is not None:
            out_dec_infrastructure = self.get_outs_dec(x_infrastructure, img_metas, reference_points, attn_mask)

        if x_vehicle is None:
            outs_dec = out_dec_infrastructure
        elif x_infrastructure is None:
            outs_dec = out_dec_vehicle
        else:
            outs_dec =  torch.stack([out_dec_vehicle, out_dec_infrastructure])
            outs_dec = torch.max(outs_dec, 0).values
