...
class A9NuscDataset(Custom3DDataset):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            test_mode=test_mode,
            # file_client_args=dict(backend='disk'),  # suren
        )
    ...
    def get_data_info(self, index: int) -> Dict[str, Any]:
        ...
        if self.modality["use_camera"]:
            ...
            for _, camera_info in info["cams"].items():
                ...
                lidar2cam = np.linalg.inv(camera2lidar)
                lidar2camera = lidar2cam[:-1, :]

                data["lidar2camera"].append(lidar2camera)    # 3x4
                data["lidar2cam"].append(lidar2cam)    # 4x4

                # camera intrinsics
                data["camera_intrinsics"].append(camera_info["camera_intrinsics"])    # 3x3

                cam_intrinsic = np.eye(4, dtype=camera_info["camera_intrinsics"].dtype)
                cam_intrinsic[:-1, :-1] = camera_info["camera_intrinsics"]
                data["cam_intrinsic"].append(cam_intrinsic)    # 4x4

                # lidar to image transform
                lidar2img = np.vstack([camera_info["lidar2image"], [0.0, 0.0 , 0.0, 1.0]])

                data["lidar2image"].append(camera_info["lidar2image"])  # 3x4
                data["lidar2img"].append(lidar2img)    # 4x4
