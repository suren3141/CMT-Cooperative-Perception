...
class A9NuscCoopDataset(Custom3DDataset):
    CLASSES = ('CAR', 'TRAILER', 'TRUCK', 'VAN', 'PEDESTRIAN', 'BUS', 'MOTORCYCLE', 'BICYCLE', 'EMERGENCY_VEHICLE')
    ...
    def get_data_info(self, index: int) -> Dict[str, Any]:
        ...
        if self.modality["use_camera"]:
            data["vehicle_img_filename"] = []
            # data["vehicle_lidar2camera"] = []
            data["vehicle_lidar2cam"] = []
            # data["vehicle_lidar2image"] = []
            data["vehicle_lidar2img"] = []
            # data["vehicle_camera_intrinsics"] = []
            data["vehicle_cam_intrinsic"] = []
            # data["vehicle_camera2lidar"] = []
            # data["vehicle_cam2lidar"] = []

            data["infrastructure_img_filename"] = []
            # data["infrastructure_lidar2camera"] = []
            data["infrastructure_lidar2cam"] = []
            # data["infrastructure_lidar2image"] = []
            data["infrastructure_lidar2img"] = []
            # data["infrastructure_camera_intrinsics"] = []
            data["infrastructure_cam_intrinsic"] = []
            # data["infrastructure_camera2lidar"] = []
            # data["infrastructure_cam2lidar"] = []

            for _, vehicle_camera_info in info["vehicle_cams"].items():
                data["vehicle_img_filename"].append(vehicle_camera_info["data_path"])

                # camera to lidar transform
                vehicle_camera2lidar = vehicle_camera_info["sensor2lidar"]
                vehicle_cam2lidar = np.vstack([vehicle_camera2lidar, [0.0, 0.0, 0.0, 1.0]])
                # data["vehicle_camera2lidar"].append(vehicle_camera2lidar)
                # data["vehicle_cam2lidar"].append(vehicle_cam2lidar)

                # lidar to camera transform
                vehicle_lidar2cam = np.linalg.inv(vehicle_cam2lidar)
                # vehicle_lidar2camera = vehicle_lidar2cam[:-1, :]
                # data["vehicle_lidar2camera"].append(vehicle_lidar2camera)    # 3x4
                data["vehicle_lidar2cam"].append(vehicle_lidar2cam)    # 4x4

                # camera intrinsics
                # data["vehicle_camera_intrinsics"].append(vehicle_camera_info["camera_intrinsics"])
                cam_intrinsic = np.eye(4, dtype=vehicle_camera_info["camera_intrinsics"].dtype)
                cam_intrinsic[:-1, :-1] = vehicle_camera_info["camera_intrinsics"]
                data["vehicle_cam_intrinsic"].append(cam_intrinsic)    # 4x4

                # lidar to image transform
                lidar2img = np.vstack([vehicle_camera_info["lidar2image"], [0.0, 0.0 , 0.0, 1.0]])

                # data["vehicle_lidar2image"].append(vehicle_camera_info["lidar2image"])  # 3x4
                data["vehicle_lidar2img"].append(lidar2img)    # 4x4

            
            for _, infrastructure_camera_info in info["infrastructure_cams"].items():
                data["infrastructure_img_filename"].append(infrastructure_camera_info["data_path"])

                # camera to lidar transform
                infrastructure_camera2lidar = infrastructure_camera_info["sensor2lidar"]
                infrastructure_cam2lidar = np.vstack([infrastructure_camera2lidar, [0.0, 0.0, 0.0, 1.0]])
                # data["infrastructure_camera2lidar"].append(infrastructure_camera2lidar)
                # data["infrastructure_cam2lidar"].append(infrastructure_cam2lidar)

                # lidar to camera transform
                infrastructure_lidar2cam = np.linalg.inv(infrastructure_cam2lidar)
                # infrastructure_lidar2camera = infrastructure_lidar2cam[:-1, :]
                # data["infrastructure_lidar2camera"].append(infrastructure_lidar2camera)    # 3x4
                data["infrastructure_lidar2cam"].append(infrastructure_lidar2cam)    # 4x4

                # camera intrinsics
                # data["infrastructure_camera_intrinsics"].append(infrastructure_camera_info["camera_intrinsics"])
                cam_intrinsic = np.eye(4, dtype=infrastructure_camera_info["camera_intrinsics"].dtype)
                cam_intrinsic[:-1, :-1] = infrastructure_camera_info["camera_intrinsics"]
                data["infrastructure_cam_intrinsic"].append(cam_intrinsic)    # 4x4

                # lidar to image transform
                lidar2img = np.vstack([infrastructure_camera_info["lidar2image"], [0.0, 0.0 , 0.0, 1.0]])

                # data["infrastructure_lidar2image"].append(infrastructure_camera_info["lidar2image"])  # 3x4
                data["infrastructure_lidar2img"].append(lidar2img)    # 4x4
