...
def create_groundtruth_database(
    ...
    elif dataset_class_name == "A9NuscDataset":
    ...
    elif dataset_class_name == "A9NuscCoopDataset":
        if not load_augmented:
            dataset_cfg.update(
                use_valid_flag=True,
                pipeline=[
    ...
    coop = True if dataset_class_name == "A9NuscCoopDataset" else False
    ...
    for j in track_iter_progress(list(range(len(dataset)))):
        ...
        if not coop:
            # use this for infrastructure only perception
            temp_name = osp.basename(example["lidar_path"])
        else:
            # use this for cooperative infrastructure+vehicle perception
            temp_name = osp.basename(example["registered_lidar_path"])
        temp_name = temp_name.split("_")
        pcd_timestamp = temp_name[0] + "_" + temp_name[1]


        if 'sample_idx' in example.keys():
            image_idx = example['sample_idx']
        elif 'pts_filename' in example.keys():
            image_idx = osp.basename(example["pts_filename"])
            image_idx = image_idx.split("_")
            image_idx = image_idx[0] + "_" + image_idx[1]        
        else:
            image_idx = pcd_timestamp
        ...
        for i in range(num_obj):
            ...
            # filename = f'{image_idx}_{names[i]}_{i}.bin'  # Suren
