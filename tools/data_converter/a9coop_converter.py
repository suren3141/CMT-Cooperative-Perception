...
class A9Coop2Nuscenes(object):
    """A9 dataset to Nuscenes converter.

        This class serves as the converter to change the A9 data to Nuscenes
        format.
    """
    ...
    def convert(self):
        ...
        for split in self.splits:
            ...
            vehicle_pcd_file_paths_sorted = sorted(
                glob(os.path.join(self.load_dir, self.map_version_to_dir[split], 'point_clouds',
            ...
            if test:
                ...
                info_path = os.path.join(self.save_dir, "{}_infos_test.pkl".format('a9_nusc_coop'))
                mmcv.dump(data, info_path)
            else:
                if split == 'training':
                    print("train sample: {}".format(len(infos_list)))
                    data = dict(infos=infos_list, metadata=metadata)
                    info_path = os.path.join(self.save_dir, "{}_infos_train.pkl".format('a9_nusc_coop'))
                    mmcv.dump(data, info_path)
                elif split == 'validation':
                    print("val sample: {}".format(len(infos_list)))
                    data = dict(infos=infos_list, metadata=metadata)
                    info_path = os.path.join(self.save_dir, "{}_infos_val.pkl".format('a9_nusc_coop'))
                    mmcv.dump(data, info_path)
