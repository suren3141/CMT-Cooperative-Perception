...
def a9_kitti_data_prep(root_path, info_prefix, version, out_dir, workers):
    if version in ['point_cloud', 'image', 'multi_modal']:
        from tools.data_converter import a9_converter as a9
        from tools.data_converter import a9_converter_pcd as a9_pcd
        splits = ['training', 'validation', 'testing']
        load_dir = osp.join(root_path)
        save_dir = osp.join(out_dir, 'kitti_format')
        os.makedirs(save_dir, exist_ok=True, mode=0o777)
        converter = a9_pcd.A92KITTI(
            splits,
            version,
            load_dir,
            save_dir)
        converter.convert()
        out_dir = osp.join(out_dir, 'kitti_format')
        #kitti.create_a9_info_file(root_path, out_dir, version, info_prefix, workers)
        #create_groundtruth_database("a9", root_path, info_prefix,
        #                            f'{out_dir}/{info_prefix}_infos_train.pkl')
    else:
        print("--help for the version choices A9: ['point_cloud', 'image', 'multi_modal']")

def a9_nusc_data_prep(root_path, info_prefix, out_dir, workers):
    from tools.data_converter import a9_converter as a9
    ...
    converter = a9.A92Nusc(splits, load_dir, save_dir)
    converter.convert()
        
    create_groundtruth_database("A9NuscDataset", save_dir, info_prefix, f'{save_dir}/{info_prefix}_infos_train.pkl')

def a9coop_data_prep(root_path, info_prefix, out_dir, workers):
    from tools.data_converter import a9coop_converter as a9coop
...
parser.add_argument(
    ...
    default='v1.0',
    required=False,
    choices=['v1.0', 'v1.0-mini', 'point_cloud', 'image', 'multi_modal'],
...
if __name__ == '__main__':
    if args.dataset == 'kitti':
        ...
        sunrgbd_data_prep(
            ...
            out_dir=args.out_dir,
            workers=args.workers)
    elif args.dataset == 'a9_kitti':
        a9_kitti_data_prep(
            root_path=args.root_path,
            info_prefix='a9_kitti',
            version=args.version,
            out_dir=args.out_dir,
            workers=args.workers)

    elif args.dataset == 'a9_nusc':
        a9_nusc_data_prep(
            root_path=args.root_path,
            info_prefix='a9_nusc',
            out_dir=args.out_dir,
    ...
    elif args.dataset == 'a9coop_nusc':
        a9coop_data_prep(
            root_path=args.root_path,
            info_prefix='a9_nusc_coop',
            out_dir=args.out_dir,
            workers=args.workers)
    else:
