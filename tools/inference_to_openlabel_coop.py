...
def main() -> None:
    ...
    if "meta_keys" in cfg.data.test.pipeline[-1].transforms[-1]:
        ...
        cfg.data.test.pipeline[-1].transforms[-1].meta_keys.append('infrastructure_lidar_path')
    else:
        cfg.data.test.pipeline[-1].transforms[-1]['meta_keys'] = ('infrastructure_lidar_path', 'box_type_3d')
    ...
    for data in tqdm(data_loader):
        ...
        try:        # BEV Fusion
            ...
            if "vehicle_points" in data:
                vehicle_points = data["vehicle_points"][0].data[0][0].cpu().numpy()
            if "infrastructure_points" in data:
                infrastructure_points = data["infrastructure_points"][0].data[0][0].cpu().numpy()
