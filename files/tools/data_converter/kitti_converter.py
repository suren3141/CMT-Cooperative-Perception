...
from .kitti_data_utils import WaymoInfoGatherer, get_kitti_image_info, get_a9_image_info
...
def _read_imageset_file_str(path):
    with open(path, 'r') as f:
        lines = f.readlines()
...
def create_a9_info_file(data_path, out_dir, version, info_prefix='a9', workers=1):
    ...
    kitti_infos_train = get_a9_image_info(data_path,
                                          training=True,
                                          label_info=True,
                                          velodyne=True,
                                          calib=False,
                                          image_ids=train_img_ids,
                                          relative_path=True)
    _calculate_num_points_in_gt(data_path, kitti_infos_train, relative_path=True)
    filename = out_dir / f'{info_prefix}_infos_train.pkl'
    print(f"a9 info train file is saved to {filename}")
    mmcv.dump(kitti_infos_train, filename)

    kitti_infos_val = get_a9_image_info(data_path,
                                        training=True,
                                        label_info=True,
                                        velodyne=True,
                                        calib=False,
                                        image_ids=val_img_ids,
                                        relative_path=True)
    _calculate_num_points_in_gt(data_path, kitti_infos_val, relative_path=True)
    filename = out_dir / f'{info_prefix}_infos_val.pkl'
    print(f"a9 info val file is saved to {filename}")
    mmcv.dump(kitti_infos_train, filename)

    filename = out_dir / f'{info_prefix}_infos_trainval.pkl'
    print(f"a9 info trainval file is saved to {filename}")
    mmcv.dump(kitti_infos_train + kitti_infos_val, filename)

    kitti_infos_test = get_a9_image_info(data_path,
                                         training=False,
                                         label_info=False,
                                         velodyne=True,
                                         calib=False,
                                         image_ids=test_img_ids,
                                         relative_path=True)
    _calculate_num_points_in_gt(data_path, kitti_infos_test, relative_path=True)
    filename = out_dir / f'{info_prefix}infos_test.pkl'
    print(f"provid21 info test file is saved to {filename}")
